...
class Injector {
  ...
  _getHash(id) {
    this.hashCounter++;
    return "0x" + sha1(`${id}:${this.hashCounter}`).slice(0,16);
  }

  // Method returns void
  _getDefaultMethodIdentifier(id){
    return `c_${sha1(id).slice((0,8))}`
  }

  // Method returns boolean: true
  _getTrueMethodIdentifier(id){
    return `c_true${sha1(id).slice((0,8))}`
  }

  // Method returns boolean: false
  _getFalseMethodIdentifier(id){
    return `c_false${sha1(id).slice((0,8))}`
  }

  _getModifierIdentifier(id){
    return `c_mod${sha1(id).slice((0,8))}`
  }
