...
class API {
  ...
  async attachToHardhatVM(provider){
    ...
    if ('_context' in cur._node) {
      if (cur._node._context._vm._vm.eei) {
        // Hardhat with dual-mode using EthereumJS VM
        cur._node._context._vm._vm.evm.events.on('step', self.collector.step.bind(self.collector))
      } else {
        // Hardhat with dual-mode using Ethereum Development Runtime (EDR) VM
        cur._node._context._vm._vm.evm.events.on('step', self.collector.edrStep.bind(self.collector))
      }
    } else {
      // Legacy Hardhat with EthereumJS VM
