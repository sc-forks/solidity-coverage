  ...
  */
  generateCoverageEnvironment() {
    ...
    try {
      ...
      } else if (truffleConfig) {
        // Instead of writing the default, put the coverage in the existing
        // config
        truffleConfig.networks || truffleConfig.networks = {};
        truffleConfig.networks.coverage || truffleConfig.networks.coverage = {
          host: 'localhost',
          nework_id: '*',
          port: this.port,
          gas: gasLimitHex,
