...
class App {
  ...
  */
  ...
  instrumentTarget() {
    ...
    try {
      shell.ls(`${this.coverageDir}/contracts/**/*.sol`).forEach(file => {
        if (!this.skipFiles.includes(file)) {
          ...
          currentFile = file;
          const contractPath = App.platformNeutralPath(file);
    ...
    App.postProcessPure(this.coverageDir);
  }
      ...
      reader
        ...
        .on('close', () => {
          ...
          try {
            ...
            const relativeMapping = this.makeKeysRelative(this.workingDir);
   ...
   */
  makeKeysRelative(root) {
    const newCoverage = {};
    // NOTE: just a personal preference but it would be much cleaner for linting to
    //       allow
    //       const { coverage } = this.coverage;
    // ref rule: object-curly-newline
    const { coverage } = this.coverage;
    Object.keys(coverage).forEach(pathKey => {
      newCoverage[path.relative(root, pathKey)] = coverage[pathKey];
    });
    return newCoverage;
  }
   ...
   */
  static platformNeutralPath(file) {
    return (isWin)
