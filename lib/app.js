...
class App {
  ...
  */
  generateCoverageEnvironment() {
    ...
    try {
      ...
      this.forceParse.forEach(item => {
        if (path.extname(item) !== '.sol')
          this.forceParseFolders.push(item);
      });
  ...
  instrumentTarget() {
    ...
    this.forceParse = this.forceParse.map(contract => `${this.coverageDir}/contracts/${contract}`);
    ...
    try {
      shell.ls(`${this.coverageDir}/contracts/**/*.sol`).forEach(file => {
        ...
        } else if (this.forceParse.includes(file) || this.inForceParseFolder(file)) {
          this.log('Skipping instrumentation of ', file);
        } else {
          this.log('Skipping instrumentation and processing of ', file);
        }
      });
    } catch (err) {
    ...
    shell
      ...
      .forEach(file => {
        ...
        if (this.deepSkip && (this.skipFiles.includes(file) || this.inSkippedFolder(file)) && !(this.forceParse.includes(file) || this.inForceParseFolder(file))) return;
