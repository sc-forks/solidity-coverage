...
class App {
  constructor(config) {
    ...
    this.abiModificationMap = [];                // ABI methods to mark as constant (Issue #146)
    this.skippedFolders = [];
  ...
  */
    ...
    try {
      shell.ls(`${this.coverageDir}/contracts/**/*.sol`).forEach(file => {
        ...
        const {
          pureFunctionNames,
          parentContractNames
        } = preprocessor.findPureMethods(contract);

        this.abiModificationMap.push({
          contractName: path.basename(canonicalPath, '.sol'),
   ...
   */
  postProcessPure(env) {
    const abiPrefix = this.coverageDir + '/build/contracts/';
    const abiPostfix = '.json';
    const parentsMap = {};
    ...
    shell.ls(`${env}/**/*.sol`).forEach(file => {
      ...
      parentsMap[contractName] = pureFunctionNames;
    ...
    this.abiModificationMap.forEach(item => {
      let abiPath = abiPrefix + item.contractName + abiPostfix;
      abiPath = this.platformNeutralPath(abiPath);

      // Search for parent methods too.
      item.parentContractNames.forEach(parentName => {
        if (parentsMap[parentName]){
          item.pureFunctionNames = item.pureFunctionNames.concat(parentsMap[parentName]);
        }
      })
      ...
      for (let j = 0; j < abiJson.abi.length; j += 1) {
        ...
        if (item.pureFunctionNames.indexOf(func.name) > -1) {
          func.constant = true;
        }
      }
      fs.writeFileSync(abiPath, JSON.stringify(abiJson, null, ' '));
    })
  }
