...
instrumenter.instrumentAssignmentExpression = function instrumentAssignmentExpression(contract, expression) {
  ...
  if (expression.right.type === 'ConditionalExpression') {
    if (expression.left.type === 'DeclarativeExpression' || expression.left.type === 'Identifier') {
      ...
      createOrAppendInjectionPoint(contract, expression.left.end, { type: 'literal', string: '; (,' + expression.left.name + ')' });
      instrumenter.instrumentConditionalExpression(contract, expression.right);
    } else if (expression.left.type === 'MemberExpression') {
      createOrAppendInjectionPoint(contract, expression.left.start, { type: 'literal', string: '(,' });
      createOrAppendInjectionPoint(contract, expression.left.end, { type: 'literal', string: ')' });
      instrumenter.instrumentConditionalExpression(contract, expression.right);
    } else {
...
instrumenter.instrumentConditionalExpression = function instrumentConditionalExpression(contract, expression) {
  ...
  contract.branchMap[contract.branchId] = {
    line: startline,
    type: 'cond-expr',
    locations: [{ start: { line: startline, column: consequentStartCol },
      end: { line: startline, column: consequentEndCol }}, { start: { line: startline, column: alternateStartCol },
      end: { line: startline, column: alternateEndCol }}],
  };
  ...
  // Wrap the consequent
  createOrAppendInjectionPoint(contract, expression.consequent.start, { type: 'openParen' });
  ...
  createOrAppendInjectionPoint(contract, expression.consequent.end, { type: 'closeParen' });

  // Wrap the alternate
  createOrAppendInjectionPoint(contract, expression.alternate.start, { type: 'openParen' });
  ...
  createOrAppendInjectionPoint(contract, expression.alternate.end, { type: 'closeParen' });
};
...
instrumenter.instrumentStatement = function instrumentStatement(contract, expression) {
  ...
  contract.statementMap[contract.statementId] = { start: { line: startline, column: startcol },
    end: { line: endline, column: endcol }};
  createOrAppendInjectionPoint(contract, expression.start, { type: 'statement', statementId: contract.statementId });
};
...
instrumenter.instrumentLine = function instrumentLine(contract, expression) {
  ...
  if (contract.instrumented.slice(lastNewLine, startchar).trim().length === 0 &&
    ...
    createOrAppendInjectionPoint(contract, lastNewLine + 1, { type: 'callEvent' });
    ...
    createOrAppendInjectionPoint(contract, expression.start, { type: 'callEvent' });
  }
...
instrumenter.instrumentFunctionDeclaration = function instrumentFunctionDeclaration(contract, expression) {
  ...
  contract.fnMap[contract.fnId] = {
    ...
    line: startline,
    loc: { start: { line: startline, column: startcol },
      end: { line: endline, column: endcol }},
  };
  createOrAppendInjectionPoint(contract, expression.start + endlineDelta + 1, { type: 'callFunctionEvent', fnId: contract.fnId });
};
...
instrumenter.addNewBranch = function addNewBranch(contract, expression) {
  ...
  contract.branchMap[contract.branchId] = {
    line: startline,
    type: 'if',
    locations: [{ start: { line: startline, column: startcol },
      end: { line: startline, column: startcol }}, { start: { line: startline, column: startcol },
      end: { line: startline, column: startcol }}],
  };
};
...
instrumenter.instrumentAssertOrRequire = function instrumentAssertOrRequire(contract, expression) {
  ...
  createOrAppendInjectionPoint(contract, expression.start, { type: 'callAssertPreEvent', branchId: contract.branchId });
  createOrAppendInjectionPoint(contract, expression.end + 1, { type: 'callAssertPostEvent', branchId: contract.branchId });
};
