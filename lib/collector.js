...
class DataCollector {
  ...
  /**
   * VM step event handler for the Ethereum Development Runtime. 
   * Detects instrumentation hashes when they are pushed to the top of the stack. 
   * This runs millions of times - trying to keep it fast.
   * @param  {Object} info  vm step info
   */
  edrStep(info){
    try {
      if (this.validOpcodes[info.opcode.name] && info.stackTop) {
        let hash = '0x' + info.stackTop.toString(16);
        this._registerHash(hash);
      }
    } catch (err) { /*Ignore*/ };
  }
