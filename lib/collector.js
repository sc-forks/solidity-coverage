...
class DataCollector {
  ...
  step(info){
    try {
      if (this.validOpcodes[info.opcode.name]) {
        let stackTop;
        if (info.stackTop) {
          stackTop = info.stackTop;
        } else if (info.stack && info.stack.length > 0) {
          const idx = info.stack.length - 1;
          stackTop = info.stack[idx];
        }
        
        if (stackTop) {
          let hash = '0x' + stackTop.toString(16);
