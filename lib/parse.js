...
parse.ContractOrLibraryStatement = function parseContractOrLibraryStatement(contract, expression) {
  ...
  if (contract.injectionPoints[injectionPoint]) {
    contract.injectionPoints[expression.start + contract.instrumented.slice(expression.start).indexOf('{') + 1].push({ type: 'eventDefinition' });
  } else {
    contract.injectionPoints[expression.start + contract.instrumented.slice(expression.start).indexOf('{') + 1] = [{ type: 'eventDefinition' }];
  }
